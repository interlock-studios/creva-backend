...
class TikTokScraper:
    ...
    async def scrape_tiktok_complete(self, url: str, options: Optional[ScrapingOptions] = None) -> Tuple[bytes, VideoMetadata, Optional[str]]:
        ...
        try:
            ...
            logger.info(f"Successfully scraped TikTok video. Video size: {len(video_content)} bytes, Transcript: {'Yes' if transcript else 'No'}")
            return video_content, metadata, transcript
            
        except Exception as e:
            logger.error(f"Failed to scrape TikTok video: {e}")
            raise
    
    async def fetch_tiktok_data(self, url: str, options: Optional[ScrapingOptions] = None) -> Dict[str, Any]:
        """
            ...
            async with httpx.AsyncClient(timeout=options.timeout) as client:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        logger.info(f"Successfully fetched TikTok data. Response size: {len(response.content)} bytes")
                        return data
                        
                    except ValueError as e:
                        raise APIError(f"Invalid JSON response: {e}", response.status_code, response.text[:500])
                
                elif response.status_code == 400:
                    raise APIError("Bad request - invalid URL or parameters", response.status_code, response.text[:500])
                    ...
                    raise APIError(f"HTTP {response.status_code}: {response.reason_phrase}", response.status_code, response.text[:500])
        ...
        try:
            ...
            if not transcript:
                ...
                for caption_info in caption_infos:
                    if caption_info.get('language_code') == 'en' or caption_info.get('lang') == 'eng-US':
            ...
            if transcript:
                ...
                if cleaned_transcript:
                    logger.info(f"Successfully extracted transcript from response. Length: {len(cleaned_transcript)} characters")
    ...
    async def get_video_info(self, url: str, options: Optional[ScrapingOptions] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"Successfully fetched video info. Transcript: {'Yes' if transcript else 'No'}")
            return result
