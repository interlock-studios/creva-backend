    ...
    async def __call__(self, request: Request, call_next):
        ...
        try:
            ...
            if self.required:
                raise HTTPException(
                    status_code=503,
                    detail="App Check verification service unavailable"
                )
            else:
                request.state.appcheck_verified = False
                return await call_next(request)
            ...
            if self.required:
                raise HTTPException(
                    status_code=500,
                    detail="Internal server error during App Check verification"
                )
            else:
                request.state.appcheck_verified = False
                return await call_next(request)
...
appcheck_scheme = HTTPBearer(scheme_name="X-Firebase-AppCheck", description="Firebase App Check token")
